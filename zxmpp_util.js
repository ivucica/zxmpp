 ...
 * namespace.
 */
var zxmpp_XMLnewDocument = function(rootTagName, namespaceURL) {
...
function zxmpp_XMLparsedDocument(txt)
{
	if(window.DOMParser)
	{
...
// by Ivan Vucica
function zxmpp_easier_attribs(xml)
{
	xml.attr = new Array();
...
if (zxmpp_ua.indexOf(" chrome/") >= 0 || zxmpp_ua.indexOf(" firefox/") >= 0 || zxmpp_ua.indexOf(' gecko/') >= 0) {
	var zxmpp_StringMaker = function () {
		this.str = "";
		this.length = 0;
...
function zxmpp_encode64(input) {
	var output = new zxmpp_StringMaker();
	var chr1, chr2, chr3;
